        ...
        if self._target_id is None:
            for event in observation.events:
                if event.actor == "E" and event.event_type in {"gesture", "gaze"} and event.target_object_id is not None:
                    ...
                    self._object_class = _object_class_from_task_spec(self.task_spec, self._target_id) or self._object_class
                    break
...
class TargetObjectOracleAgent:
    """Oracle baseline that acts on the task's resolved target object."""

    name = "target_object_oracle"

    def reset(self, task_spec: TaskSpec) -> None:
        self.task_spec = task_spec
        target = task_spec.objects.get("target", {})
        self._target_id = task_spec.success.get("target_object_id") or target.get("id")
        self._object_class = str(target.get("class_name", "object"))
        self._phase = "move"

    def act(self, observation: Observation) -> Action:
        if self._target_id is None:
            return Action(action_type="ask", utterance="Which mug do you mean?")
        ...
        if self._phase == "move":
            ...
            return Action(
                action_type="move_to",
                target_object_id=int(self._target_id),
                params={"object_class": self._object_class},
            )
...
def _object_class_from_task_spec(task_spec: TaskSpec, object_id: int) -> str | None:
    return _object_class_from_value(task_spec.objects, int(object_id))


def _object_class_from_value(value: object, object_id: int) -> str | None:
    if isinstance(value, dict):
        if value.get("id") is not None and int(value["id"]) == object_id:
            return str(value.get("class_name", "object"))
        for item in value.values():
            found = _object_class_from_value(item, object_id)
